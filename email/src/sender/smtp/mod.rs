...
// fn to_msg_builder<'a>(msg: Message<'a>) -> MessageBuilder<'a> {
//     let mut builder = MessageBuilder::new();

//     for header in msg.headers() {
//         builder = builder.header(header.name(), header::display_value(&header.value));
//     }

//     match msg.root_part().body {
//         PartType::Text(text) => {
...
//             let default_ctype = ContentType::new("application/octet-stream");
//             let ctype = msg
//                 .root_part()
//                 .content_type()
//                 .and_then(|ctype| {
//                     let stype = ctype.subtype();
//                     let ctype = ctype.ctype();
//                     stype.map(|stype| ContentType::new(format!("{ctype}/{stype}")))
//                 })
//                 .unwrap_or(default_ctype);
//             let fname = msg.root_part().attachment_name().unwrap_or("noname");
//             builder = builder.attachment(ctype, fname, BodyPart::Binary(data));
//         }
